...
import com.tama.q_municate_core.models.ParcelableQBDialog;
...
public class ChatUtils {
  ...
  public static ArrayList<Integer> createOccupantsIdsFromPrivateMessage(int currentUserId,
      int senderId) {
  ...
  public static QBChatDialog getExistPrivateDialog(DataManager dataManager, int opponentId) {
    ...
    if (dialogOccupant != null) {
      return dataManager.getQBChatDialogDataManager()
          .getByDialogId(dialogOccupant.getDialog().getDialogId());
    } else {
      return null;
    }
  }
  ...
  public static String getFullNamesFromOpponentIds(DataManager dataManager,
  ...
  private static String getFullNamesFromOpponentIdsList(DataManager dataManager,
      List<Integer> occupantsIdsList) {
    StringBuilder stringBuilder = new StringBuilder(occupantsIdsList.size());
    for (Integer id : occupantsIdsList) {
      stringBuilder.append(getFullNameById(dataManager, id)).append(OCCUPANT_IDS_DIVIDER)
          .append(" ");
    }
    return stringBuilder.toString()
        .substring(ConstsCore.ZERO_INT_VALUE, stringBuilder.length() - 2);
  ...
  public static List<ParcelableQBDialog> qBDialogsToParcelableQBDialogs(
      List<QBChatDialog> dialogList) {
    List<ParcelableQBDialog> parcelableDialogList = new ArrayList<ParcelableQBDialog>(
        dialogList.size());
  ...
  public static List<QBChatDialog> parcelableQBDialogsToQBDialogs(
      List<ParcelableQBDialog> parcelableQBDialogsList) {
  ...
  public static QMUser getOpponentFromPrivateDialog(QMUser currentUser,
      List<DialogOccupant> occupantsList) {
  ...
  public static List<Message> createTempLocalMessagesList(DataManager dataManager,
    ...
    for (QBChatDialog qbDialog : qbDialogsList) {
      ...
      if (currentDialog != null && qbDialog.getDialogId().equals(currentDialog.getDialogId())) {
        Log.d(TAG, "Fix double message currentDialog = " + currentDialog + " currentDialogID = "
            + currentDialog.getDialogId());
        continue;
      }
...
//            .getDialogOccupant(qbDialog.getDialogId(), AppSession.getSession().getUser().getId());
//      }

      DialogOccupant dialogOccupant = getDialogOccupant(dataManager, qbDialog);

      long tempMessageId = qbDialog.getDialogId().hashCode();
      Message message = createTempLocalMessage(tempMessageId, dialogOccupant,
          qbDialog.getLastMessage(), qbDialog.getLastMessageDateSent(), State.TEMP_LOCAL);
...
//          messagesList.add(createTempLocalMessage(--tempMessageId, dialogOccupant, null,
...
//      }
  ...
  public static List<Message> createTempUnreadMessagesList(DataManager dataManager,
    ...
    for (QBChatDialog qbDialog : qbDialogsList) {
      if (currentDialog != null && qbDialog.getDialogId().equals(currentDialog.getDialogId())) {
        Log.d(TAG, "currentDialog = " + currentDialog + " currentDialogID = " + currentDialog
            .getDialogId());
        continue;
      }
      ...
      for (int i = 0; i < unreadCount; i++) {
        Message message = createTempLocalMessage(tempMessageId++, dialogOccupant,
            qbDialog.getLastMessage(), createdData++, State.TEMP_LOCAL_UNREAD);
        messagesList.add(message);
      }
    }
  ...
  private static DialogOccupant getDialogOccupant(DataManager dataManager, QBChatDialog qbDialog) {
    ...
    if (dialogOccupant == null && onlyMeInDialog) {
      QMUser user = QMUserService.getInstance().getUserCache()
          .get((long) AppSession.getSession().getUser().getId());
      DbUtils.saveDialogOccupant(dataManager,
  ...
  public static Message createTempLocalMessage(long messageId, DialogOccupant dialogOccupant,
  ...
  private static Message createTempLocalMessage(long messageId, DialogOccupant dialogOccupant,
      String body, long createdDate, State state) {
  ...
  //avelacrac
//  public static List<QBChatDialog> createQBDialogsListFromDialogsList(DataManager dataManager,
...
//  }
  ...
  public static List<DialogOccupant> createDialogOccupantsList(DataManager dataManager,
      QBChatDialog qbDialog, boolean onlyNewOccupant) {
    ...
    for (Integer userId : qbDialog.getOccupants()) {
      ...
      if (onlyNewOccupant) {
        dialogOccupant = dataManager.getDialogOccupantDataManager()
            .getDialogOccupant(qbDialog.getDialogId(), userId);
        if (dialogOccupant == null) {
          dialogOccupant = createDialogOccupant(dataManager, qbDialog.getDialogId(),
...
//          dialogOccupantsList.add(dialogOccupant);
  ...
  public static List<Integer> getUsersIdsFromDialogOccupantsList(
...
//  //chka
//  public static QBChatDialog createQBDialogFromLocalDialogWithoutLeaved(DataManager dataManager,
//      Dialog dialog) {
//    List<DialogOccupant> dialogOccupantsList = dataManager.getDialogOccupantDataManager()
...
//
//  //chka
//  private static QBChatDialog createQBDialogFromLocalDialog(Dialog dialog,
//      List<DialogOccupant> dialogOccupantsList) {
...
//  }
  ...
  public static Message createLocalMessage(Context context, QBChatMessage qbChatMessage, DialogOccupant dialogOccupant, State state, int ocupantCount) {
    ...
    if(dialogOccupant.getDialog().getType()== Dialog.Type.GROUP){
//      if (ocupantCount==qbChatMessage.getReadIds().size()) {
//        state = State.READ;
//      } else {
        state = State.DELIVERED;
//      };//Avetik
      message.setState(state);
//      message.setState( ocupantCount==qbChatMessage.getReadIds().size()? State.READ : State.DELIVERED);
      Log.d(TAG,"ocupantCount = " + ocupantCount);
    }else{
...
//
    ...
    if(qbChatMessage.getProperties().containsKey(context.getResources().getString(R.string.snap_message_key))) {
      message.setSnapMessage(qbChatMessage.getProperties().get(context.getResources().getString(R.string.snap_message_key)).equals(context.getResources().getString(R.string.snap_message_value)));//Avetik
      message.setSnapMessageTime(Integer.valueOf(qbChatMessage.getProperties().get(context.getResources().getString(R.string.snap_message_time_key))));//Avetik
    }
    return message;//steg arden darav REED Chpetqa tenc lini
  }

  public static Message createLocalMessage(Context context,QBChatMessage qbChatMessage, DialogOccupant dialogOccupant, State state) {
    long dateSent = getMessageDateSent(qbChatMessage);
    ////nott
    Message message = new Message();
    message.setMessageId(qbChatMessage.getId());
  ...
  public static long getMessageDateSent(QBChatMessage qbChatMessage) {
    long dateSent;
    String dateSentString = (String) qbChatMessage
        .getProperty(ChatNotificationUtils.PROPERTY_DATE_SENT);
    try {
      dateSent =
          dateSentString != null ? Long.parseLong(dateSentString) : qbChatMessage.getDateSent();
...
//  }
  ...
  public static Attachment createLocalAttachment(QBAttachment qbAttachment, Context context,
    ...
    if (qbAttachment.getType().equalsIgnoreCase(QBAttachment.LOCATION_TYPE)) {
      ...
      remoteUrl = LocationUtils
          .getRemoteUri(qbAttachment.getData(), LocationUtils.defaultUrlLocationParams(context));
  ...
  public static DialogNotification createLocalDialogNotification(Context context,
    ...
    String chatNotificationTypeString = qbChatMessage
        .getProperty(ChatNotificationUtils.PROPERTY_NOTIFICATION_TYPE).toString();

    if (chatNotificationTypeString != null) {

      int chatNotificationTypeInt = Integer.parseInt(chatNotificationTypeString);
      NotificationType chatNotificationType = NotificationType
          .parseByValue(chatNotificationTypeInt);
  ...
  public static List<CombinationMessage> getCombinationMessagesListFromMessagesList(
  ...
  public static List<CombinationMessage> getCombinationMessagesListFromDialogNotificationsList(
      List<DialogNotification> dialogNotificationsList) {
    List<CombinationMessage> combinationMessagesList = new ArrayList<>(
        dialogNotificationsList.size());
  ...
  public static List<DialogNotification> readAllDialogNotification(
      List<DialogNotification> dialogNotificationsList, QBUser currentQbUser) {
    List<DialogNotification> updateDialogNotificationsList = new ArrayList<>(
        dialogNotificationsList.size());

    for (DialogNotification dialogNotification : dialogNotificationsList) {
      if (!State.READ.equals(dialogNotification.getState())
          && currentQbUser.getId().intValue() != dialogNotification.getDialogOccupant().getUser()
          .getId().intValue()) {
  ...
  public static List<Message> readAllMessages(List<Message> messagesList, QBUser currentQbUser) {
    ...
    for (Message message : messagesList) {
      if (!State.READ.equals(message.getState())
          && currentQbUser.getId().intValue() != message.getDialogOccupant().getUser().getId()
          .intValue()) {
  ...
  public static List<CombinationMessage> createLimitedCombinationMessagesList(
      List<Message> messagesList,
    ...
    int indexStart =
        combinationMessagesList.size() < limit ? 0 : combinationMessagesList.size() - limit;
    int indexEnd = combinationMessagesList.size();

    return combinationMessagesList.subList(indexStart, indexEnd);
  }

  public static String getDialogLastMessage(String defaultLasMessage, Message message,
  ...
  //avelacrac
  public static String getDialogLastMessage(String defaultLasMessage, String snapMessageText,
      Message message, DialogNotification dialogNotification) {
    String lastMessage = "";

    if (message == null && dialogNotification != null) {
      lastMessage = defaultLasMessage;
    } else if (dialogNotification == null && message != null) {
      if (message.getSnapMessage()) {
        lastMessage = snapMessageText;
      } else {
        lastMessage = message.getBody();
      }
    } else if (message != null) {
      Log.d(TAG, "SSSSSSSSSSSSSSSSSSTEEEEEEEEGAAAAAAAAAAAAAAAAAAA  ==  " + message.getSnapMessage());
      lastMessage = message.getCreatedDate() > dialogNotification.getCreatedDate()
          ? (message.getSnapMessage() ? snapMessageText : message.getBody()) : defaultLasMessage;
    }
  ...
  public static long getDialogMessageCreatedDate(boolean lastMessage, Message message,
      DialogNotification dialogNotification) {
    long createdDate = 0;
    ...
    if (message == null && dialogNotification == null) {
      ...
      createdDate = lastMessage
          ? (message.getCreatedDate() > dialogNotification.getCreatedDate() ? message
          .getCreatedDate() : dialogNotification.getCreatedDate())
          : (message.getCreatedDate() < dialogNotification.getCreatedDate() ? message
              .getCreatedDate() : dialogNotification.getCreatedDate());
    } else if (message != null && dialogNotification == null) {
  ...
  //hanel
//  public static List<Dialog> fillTitleForPrivateDialogsList(String titleForDeletedUser,
//      DataManager dataManager,
...
//  }
  ...
  public static int getOnlineDialogOccupantsCount(QBFriendListHelper friendListHelper,
      List<Integer> occupantIdsList) {
    int onlineOccupantsCount = 0;

    for (int userId : occupantIdsList) {
      if (userId == AppSession.getSession().getUser().getId() || friendListHelper
          .isUserOnline(userId)) {
  ...
  public static List<DialogOccupant> getUpdatedDialogOccupantsList(DataManager dataManager,
      String dialogId, List<Integer> dialogOccupantIdsList, DialogOccupant.Status status) {
    ...
    for (Integer userId : dialogOccupantIdsList) {
      ...
      DialogOccupant dialogOccupant = getUpdatedDialogOccupant(dataManager, dialogId, status,
          userId);
      if (dialogOccupant == null) {
  ...
  public static DialogOccupant getUpdatedDialogOccupant(DataManager dataManager, String dialogId,
    ...
    DialogOccupant dialogOccupant = dataManager.getDialogOccupantDataManager()
        .getDialogOccupant(dialogId, userId);
    if (dialogOccupant != null) {
      dialogOccupant.setStatus(status);
    }
    return dialogOccupant;
  ...
  public static DialogOccupant createDialogOccupant(DataManager dataManager, String dialogId,
      QMUser user) {
