...
import com.tama.q_municate_db.models.UserRequest;
...
//        this.privateChatHelper = privateChatHelper;
    ...
    private synchronized void sendNotificationToFriend(QBChatMessage qbChatMessage, int userId) throws QBResponseException {
        ...
        if (qbDialog != null) {
//            chatHelper.sendChatMessage(qbChatMessage, qbDialog); //Hayk
    ...
    public Collection<Integer> updateFriendList() throws QBResponseException {
        ...
        if (roster != null) {
//            if (!roster.getEntries().isEmpty()) {//mut kod
//                int A = 100;
                userIdsList = createFriendList(); //Avetik
...
//            }
    ...
    private Collection<Integer> createFriendList(// Avetik
            ) throws QBResponseException {


        Collection<Integer> friendIdList = getFriendListFromPhoneNumber();

        loadAndSaveUsers(friendIdList, UserRequest.RequestStatus.OUTGOING);

        return friendIdList;
    }


    private Collection<Integer> createFriendList( Collection<QBRosterEntry> rosterEntryCollection) throws QBResponseException {
    ...
    private Collection<Integer> getFriendListFromPhoneNumber(){//Avetik
        Collection<Integer> friendList = new ArrayList<>();
        QBPagedRequestBuilder requestBuilder = new QBPagedRequestBuilder();
        requestBuilder.setPage(1);
        requestBuilder.setPerPage(ConstsCore.FL_FRIENDS_PER_PAGE);

        Collection<String> phone = getPhoneNumberArr();

        Bundle requestParams = new Bundle();

        Collection<QBUser> userList = null;
        try {
            userList = QBUsers.getUsersByPhoneNumbers(phone,requestBuilder, requestParams).perform();
    ...
    private Collection<String> getPhoneNumberArr() {
        ...
        if (c.getCount() <= 0) {
            Toast
                .makeText(getApplicationContext(), "No Phone Contact Found..!", Toast.LENGTH_SHORT).show();
        } else {
            while (c.moveToNext()) {
                String phoneNumber = c.getString(c.getColumnIndex(   ContactsContract.CommonDataKinds.Phone.NUMBER));          //Phone number
...
//    private Collection<String> getPhoneNumberArr(){//Avetik
//        Collection<String> phone = new ArrayList<>();
...
//        List<QMUser> usersList = (List<QMUser>) restHelper.loadUsers(friendIdsList); //Hayk
        QBPagedRequestBuilder requestBuilder = new QBPagedRequestBuilder();
        requestBuilder.setPage(ConstsCore.USERS_PAGE_NUM);
        requestBuilder.setPerPage(ConstsCore.USERS_PER_PAGE);
        List<QMUser> usersList = QMUserService.getInstance().getUsersByIDsSync(friendIdsList, requestBuilder);
...
//        dataManager.getUserRequestDataManager().createOrUpdate(user);
//    }
...
//        Collection<QBUser> userList = null;
////        try {
////            userList = QBUsers.getUsersByPhoneNumbers(phone,requestBuilder, requestParams);
////            for(QBUser user : userList)
////                friendList.add(user.getId());
////        } catch (QBResponseException e) {
////            e.printStackTrace();
////        }
//////        try {
//            userList = (Collection<QBUser>) QBUsers
//                .getUsersByPhoneNumbers(phone,requestBuilder, requestParams);
//            for(QBUser user : userList)
//                friendList.add(user.getId());
//////        } catch (QBResponseException e) {
//////            e.printStackTrace();
