...
//        DbUtils.saveMessageOrNotificationToCache(context, dataManager, dialogId, qbChatMessage, null, true, currentDialog.getOccupants().size());
...
//        List<QBChatDialog> qbDialogsList = QBChatService.getChatDialogs(null, qbRequestGetBuilder, returnedBundle);
//
...
//        try {
//            QBChatService.deleteMessage(messageId);
//
//        } catch (QBResponseException e) {
//            ErrorUtils.logError(e);
//        }
//        DbUtils.deleteMessageLocal(dataManager, Long.parseLong(messageId));
    }

    protected QBChatMessage getQBChatMessage(String body, QBFile qbFile, boolean isSnap, Integer snapTimeValue) {//4444
        long time = DateUtilsCore.getCurrentTime();
        ...
        chatMessage.setProperty(ChatNotificationUtils.PROPERTY_SAVE_TO_HISTORY,
            String.valueOf(ChatNotificationUtils.VALUE_SAVE_TO_HISTORY));
        if(isSnap){
    ...
    private QBAttachment getAttachment(QBFile file) {
        // TODO temp value
        String contentType = file.getContentType();//Avetik
        QBAttachment attachment;//Avetik
...
//        String contentType = "image/jpeg";//Avetik
//        QBAttachment attachment = new QBAttachment(QBAttachment.PHOTO_TYPE);//Avetik
    ...
    public QBChatDialog createPrivateChatOnRest(int opponentId) throws QBResponseException {
        QBChatDialog dialog = privateChatManager.createDialog();
        return dialog;
    }
    ...
    private void sendNotificationBroadcast(String action, QBChatMessage chatMessage, QMUser user, String dialogId,
        ...
        String messageBody;
        if(chatMessage.getProperties().containsKey(context.getResources().getString(R.string.snap_message_key)) &&
                chatMessage.getProperties().get(context.getResources().getString(R.string.snap_message_key)).equals(context.getResources().getString(R.string.snap_message_value))) {
...
//            QMUser user = dataManager.getUserDataManager().get(qbChatMessage.getSenderId());
//            if (user != null) {
...
//            QBChatService.markMessagesAsRead(dialogId, messagesIdsList);
    ...
    public void deleteMessage(String messageId) {
        try {
            QBRestChatService.deleteMessage(messageId);
...
//        QBAttachment attachment = new QBAttachment(QBAttachment.PHOTO_TYPE);//Avetik
    ...
    public void updateStatusMessageReadServer(String dialogId, CombinationMessage combinationMessage,
        ...
        if (fromPrivate) {
            ...
            StringifyArrayList<String> messagesIdsList = new StringifyArrayList<String>();
            messagesIdsList.add(combinationMessage.getMessageId());
            QBRestChatService.markMessagesAsRead(dialogId, messagesIdsList);
        }
    }
