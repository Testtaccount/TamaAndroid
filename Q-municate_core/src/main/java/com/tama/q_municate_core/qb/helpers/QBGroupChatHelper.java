...
//            DbUtils.saveMessageOrNotificationToCache(context, dataManager, dialogId, qbChatMessage, State.DELIVERED, true,currentDialog.getOccupants().size());
        }
    ...
    public void sendGroupMessage(String roomJidId, String message, boolean isSnap, int snapTime) throws Exception {
        QBChatMessage chatMessage = getQBChatMessage(message, null, isSnap, snapTime);
...
//            existingDialog = ChatUtils.createQBDialogFromLocalDialog(dataManager, dataManager.getDialogDataManager().getByDialogId(dialogId));
    ...
    public void sendGroupMessageWithAttachImage(String roomJidId, QBFile file, boolean isSnap, int snapTime) throws QBResponseException {
        String body;
        if(file.getContentType().contains("image/")) {
            body = context.getString(R.string.dlg_attached_last_message_image);
        }else{
            body = context.getString(R.string.dlg_attached_last_message_video);
        }
        QBChatMessage chatMessage = getQBChatMessage(body, file, isSnap,
                snapTime);
...
//        updateDialog(ChatUtils.createQBDialogFromLocalDialog(dataManager, dialog), requestBuilder);
    ...
    public QBChatDialog updateDialog(QBChatDialog dialog, File inputFile) throws QBResponseException {
        QBFile file = (QBFile) QBContent.uploadFileTask(inputFile, true, (String) null);
