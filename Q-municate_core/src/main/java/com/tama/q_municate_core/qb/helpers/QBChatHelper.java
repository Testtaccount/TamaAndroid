...
import com.tama.q_municate_core.utils.DateUtilsCore;
import com.tama.q_municate_core.utils.DbUtils;
import com.tama.q_municate_core.utils.FinderUnknownUsers;
import com.tama.q_municate_core.utils.MediaUtils;
import com.tama.q_municate_core.utils.MimeTypeAttach;
...
import com.tama.q_municate_db.utils.DialogTransformUtils;
...
//  private GroupChatMessagesStatusListener groupChatMessagesStatusListener;
...
//    groupChatMessagesStatusListener = new GroupChatMessagesStatusListener();
  ...
  public synchronized void initCurrentChatDialog(QBChatDialog dialog, Bundle additional)
  ...
  private void initMainChatListeners() {
    chatService.getMessageStatusesManager()
        .addMessageStatusListener(privateChatMessagesStatusListener);
//    chatService.getMessageStatusesManager()
//        .addMessageStatusListener(groupChatMessagesStatusListener);
  ...
  public void sendChatMessage(String message, boolean isSnap, Integer snapTimeValue)
      throws QBResponseException {
    QBChatMessage qbChatMessage = getQBChatMessage(message, isSnap, snapTimeValue);
    sendAndSaveChatMessage(qbChatMessage, currentDialog);
  }

  public void sendMessageWithAttachment(Attachment.Type attachmentType, Object attachmentObject,
      String localPath, boolean isSnap, int snapTime) throws QBResponseException {
    String messageBody = "";
    ...
    QBChatMessage message = getQBChatMessage(messageBody, isSnap, snapTime);
    message.addAttachment(attachment);

    sendAndSaveChatMessage(message, currentDialog);
  }

  public void sendAndSaveChatMessage(QBChatMessage qbChatMessage, QBChatDialog chatDialog)
      throws QBResponseException {
    ...
    if (QBDialogType.PRIVATE.equals(chatDialog.getType())) {
      ...
      DbUtils.saveMessageOrNotificationToCache(context, dataManager, chatDialog.getDialogId(), qbChatMessage, State.SYNC, true, currentDialog.getOccupants().size());
    }
  }

  public void sendChatMessage(QBChatMessage message, QBChatDialog chatDialog)
      throws QBResponseException {
    ...
    if (QBDialogType.GROUP.equals(chatDialog.getType())) {
      ...
      message.setSenderId(AppSession.getSession().getUser().getId());
    }
  ...
  protected void addNecessaryPropertyForQBChatMessage(QBChatMessage qbChatMessage,
      String dialogId) {
    long time = DateUtilsCore.getCurrentTime();
    qbChatMessage.setDialogId(dialogId);
    qbChatMessage
        .setProperty(ChatNotificationUtils.PROPERTY_DATE_SENT, time + ConstsCore.EMPTY_STRING);
  }

  public List<QBChatDialog> getDialogs(QBRequestGetBuilder qbRequestGetBuilder,
      Bundle returnedBundle) throws QBResponseException {
    List<QBChatDialog> qbDialogsList = QBRestChatService.getChatDialogs(null, qbRequestGetBuilder)
        .perform();
  ...
  public List<QBChatMessage> getDialogMessages(QBRequestGetBuilder customObjectRequestBuilder,
    ...
    if (qbMessagesList != null && !qbMessagesList.isEmpty()) {
      //Hayk chat dublicate here
      DbUtils.saveMessagesToCache(context, dataManager, qbMessagesList, qbDialog.getDialogId(),currentDialog.getOccupants().size());
    }
  ...
  public void deleteMessage(final String messageId) {
    QBRestChatService.deleteMessage(messageId, true).performAsync(
        new QBEntityCallback<Void>() {
          @Override
          public void onSuccess(Void aVoid, Bundle bundle) {
            Log.d("testt", "Deleted!!!!!!!!!");
            DbUtils.deleteMessageLocal(dataManager, messageId);
  ...
  public void saveDialogsToCache(List<QBChatDialog> qbDialogsList, boolean allDialogs) {
    if (qbDialogsList != null && !qbDialogsList.isEmpty()) {
      FinderUnknownUsers finderUnknownUsers = new FinderUnknownUsers(context,
          AppSession.getSession().getUser(), qbDialogsList);
  ...
  private void prepareAndSendNotificationsToOpponents(String dialogId) throws QBResponseException {
    ...
    ArrayList<Integer> actualOpponentsIds =
        ChatUtils
            .createOccupantsIdsFromDialogOccupantsList(dataManager.getDialogOccupantDataManager()
  ...
  private QBChatMessage getQBChatMessage(String body, boolean isSnap,
      Integer snapTimeValue) { //Hayk
    long time = DateUtilsCore.getCurrentTime();
    ...
    if (isSnap) {
      chatMessage.setProperty(ChatNotificationUtils.SNAP_CHAT_MESSAGE_KEY,
          ChatNotificationUtils.SNAP_CHAT_MESSAGE_VALUE);
      chatMessage.setProperty(ChatNotificationUtils.SNAP_CHAT_MESSAGE_TIME,
          String.valueOf(snapTimeValue));
    }
    return chatMessage;
  }
  ...
  private QBAttachment getAttachmentImage(QBFile file, String localPath) {
    QBAttachment attachment = getAttachment(file, QBAttachment.IMAGE_TYPE,
        MimeTypeAttach.IMAGE_MIME);
  ...
  private QBAttachment getAttachmentAudio(QBFile file, String localPath) {
    QBAttachment attachment = getAttachment(file, QBAttachment.AUDIO_TYPE,
        MimeTypeAttach.AUDIO_MIME);
  ...
  private QBAttachment getAttachmentVideo(QBFile file, String localPath) {
    QBAttachment attachment = getAttachment(file, QBAttachment.VIDEO_TYPE,
        MimeTypeAttach.VIDEO_MIME);
  ...
  public QBChatDialog createPrivateChatOnRest(int opponentId) throws QBResponseException {
    QBChatDialog dialog = QBRestChatService
        .createChatDialog(DialogUtils.buildPrivateDialog(opponentId)).perform();
    return dialog;
  }
  ...
  protected void checkForSendingNotification(boolean ownMessage, QBChatMessage qbChatMessage,
      QMUser user,
    ...
    if (currentDialog != null) {
      if (!ownMessage && !currentDialog.getDialogId().equals(dialogId)) {
        sendNotificationBroadcast(QBServiceConsts.GOT_CHAT_MESSAGE_LOCAL, qbChatMessage, user,
            dialogId, isPrivateChat);
      }
    } else if (!ownMessage) {
      sendNotificationBroadcast(QBServiceConsts.GOT_CHAT_MESSAGE_LOCAL, qbChatMessage, user,
          dialogId,
  ...
  private void sendNotificationBroadcast(String action, QBChatMessage chatMessage, QMUser user,
      String dialogId,
      boolean isPrivateMessage) {
    Intent intent = new Intent(action);
    String messageBody = chatMessage.getBody();
    if (chatMessage.getProperties()
        .containsKey(context.getResources().getString(R.string.snap_message_key)) &&
        chatMessage.getProperties().get(context.getResources().getString(R.string.snap_message_key))
            .equals(context.getResources().getString(R.string.snap_message_value))) {
      messageBody = context.getResources().getString(R.string.snap_text);
    } else {
    ...
    if (!CollectionsUtil.isEmpty(chatMessage.getAttachments())) {
      ...
      extraChatMessage = context.getResources()
          .getString(R.string.was_attached, attachment.getType());
    } else {
  ...
  protected Message parseReceivedMessage(QBChatMessage qbChatMessage) {
    ...
    if (dialogOccupant == null) {
      ...
      QMUser user = QMUserService.getInstance().getUserCache()
          .get((long) qbChatMessage.getSenderId());
      if (user != null) {
...
//            }
      if (getAttachmentType(qbChatMessage.getAttachments())
          .equalsIgnoreCase(QBAttachment.PHOTO_TYPE)) {
        attachment.setType(Attachment.Type.IMAGE);
      } else {
        attachment.setType(Attachment.Type
            .valueOf(getAttachmentType(qbChatMessage.getAttachments()).toUpperCase()));
      }
  ...
  public void updateStatusNotificationMessageRead(QBChatDialog chatDialog,
      CombinationMessage combinationMessage) throws Exception {
    updateStatusMessageReadServer(chatDialog, combinationMessage, true);
    DbUtils.updateStatusNotificationMessageLocal(dataManager,
        combinationMessage.toDialogNotification());
  }

  public void updateStatusMessageRead(QBChatDialog chatDialog,
  ...
  public void updateStatusMessageReadServer(QBChatDialog chatDialog,
      CombinationMessage combinationMessage,
      boolean isOnline) throws Exception {
    if (isOnline) {
      chatDialog.initForChat(chatService);
  ...
  private void createGroupDialogByNotification(QBChatMessage qbChatMessage,
      DialogNotification.Type notificationType) {
    qbChatMessage.setBody(context.getString(R.string.cht_notification_message));

    QBChatDialog qbDialog = ChatNotificationUtils
        .parseDialogFromQBMessage(context, qbChatMessage, qbChatMessage.getBody(),
            QBDialogType.GROUP);
    ...
    DialogNotification dialogNotification = ChatUtils
        .convertMessageToDialogNotification(parseReceivedMessage(qbChatMessage));
  ...
  public synchronized void tryJoinRoomChatsPage(List<QBChatDialog> qbDialogsList,
      boolean needClean) {
  ...
  public void joinRoomChat(QBChatDialog dialog, QBEntityCallback<Void> callback) {
    ...
    if (!dialog.isJoined()) { //join only to unjoined dialogs
      dialog.join(history(),
          callback); //join asynchronously, this doesn't block current thread to enqueue join for next dialog
    }
  }
  ...
  public void sendGroupMessageToFriends(QBChatDialog qbDialog,
      DialogNotification.Type notificationType,
    ...
    QBChatMessage chatMessage = ChatNotificationUtils
        .createGroupMessageAboutUpdateChat(context, qbDialog,
            notificationType, occupantsIdsList, leavedFromDialog);
    sendChatMessage(chatMessage, qbDialog);
  }

  public QBChatDialog addUsersToDialog(String dialogId, List<Integer> userIdsList)
      throws Exception {
    ...
    requestBuilder
        .push(com.quickblox.chat.Consts.DIALOG_OCCUPANTS, occupantsIdsList.getItemsAsString());
    return updateDialog(chatDialog, requestBuilder);
  }

  public void removeUsersFromDialog(QBChatDialog chatDialog, List<Integer> userIdsList)
      throws QBResponseException {
    QBRequestUpdateBuilder requestBuilder = new QBRequestUpdateBuilder();
  ...
  private QBChatDialog updateDialog(QBChatDialog chatDialog, QBRequestUpdateBuilder requestBuilder)
      throws QBResponseException {
    QBChatDialog updatedDialog = QBRestChatService.updateGroupChatDialog(chatDialog, requestBuilder)
        .perform();
    return updatedDialog;
  }

  public QBChatDialog createGroupChat(String name, List<Integer> friendIdsList, String photoUrl)
      throws Exception {
    ArrayList<Integer> occupantIdsList = (ArrayList<Integer>) ChatUtils
        .getOccupantIdsWithUser(friendIdsList);
    ...
    QBChatMessage chatMessage = ChatNotificationUtils
        .createGroupMessageAboutCreateGroupChat(context, qbDialog, photoUrl);
    sendChatMessage(chatMessage, qbDialog);
  ...
  public void sendSystemMessageAboutCreatingGroupChat(QBChatDialog dialog,
  ...
  private void sendSystemMessageAboutCreatingGroupChat(QBChatDialog dialog, Integer friendId)
      throws Exception {
  ...
  private void updateGroupDialogByNotification(QBChatMessage qbChatMessage) {
    ...
    if (qbDialog == null) {
      qbDialog = ChatNotificationUtils
          .parseDialogFromQBMessage(context, qbChatMessage, QBDialogType.GROUP);
    }
...
//                    not notify and return if currentUser deleted groupDialog
    ...
    if (chatDialog == null) {
      chatDialog = ChatNotificationUtils
          .parseDialogFromQBMessage(context, chatMessage, QBDialogType.PRIVATE);
    ...
    DbUtils.updateDialogModifiedDate(dataManager, chatDialog,
        ChatUtils.getMessageDateSent(chatMessage), false);
    DbUtils.saveMessageOrNotificationToCache(context, dataManager, dialogId, chatMessage,
        !ownMessage
            ? State.DELIVERED
            : State.SYNC,
        needNotifyObserver, currentDialog != null ? currentDialog.getOccupants().size() : 0);
  ...
  private boolean isNotificationToGroupChat(QBChatMessage chatMessage) {
    String updatedInfo = (String) chatMessage
        .getProperty(ChatNotificationUtils.PROPERTY_ROOM_UPDATE_INFO);
    return updatedInfo != null;
  }

  private boolean isNotificationDeletedGroupChat(QBChatMessage chatMessage) {
    String deletedOccupantsIdsString = (String) chatMessage
        .getProperty(ChatNotificationUtils.PROPERTY_ROOM_DELETED_OCCUPANTS_IDS);
  ...
  private void friendRequestMessageReceived(QBChatMessage qbChatMessage,
    ...
    QBChatDialog chatDialog = dataManager.getQBChatDialogDataManager().getByDialogId(dialogId);
    if (chatDialog == null) {
      QBChatDialog newChatDialog = ChatNotificationUtils
          .parseDialogFromQBMessage(context, qbChatMessage, QBDialogType.PRIVATE);
      ArrayList<Integer> occupantsIdsList = ChatUtils
          .createOccupantsIdsFromPrivateMessage(chatCreator.getId(), qbChatMessage.getSenderId());
    ...
    DialogOccupant dialogOccupant = dataManager.getDialogOccupantDataManager()
        .getDialogOccupant(dialogId, qbChatMessage.getSenderId());
    DbUtils
        .saveDialogNotificationToCache(context, dataManager, dialogOccupant, qbChatMessage, false);
  ...
  private class AllChatMessagesListener implements QBChatDialogMessageListener {
    ...
    @Override
    public void processMessage(final String dialogId, final QBChatMessage qbChatMessage,
    ...
    @Override
    public void processError(String dialogId, QBChatException exception,
        QBChatMessage qbChatMessage, Integer integer) {
  ...
  private class PrivateChatMessagesStatusListener implements QBMessageStatusListener {
    ...
    @Override
    public void processMessageRead(final String messageId, String dialogId, Integer userId) {
      QBChatDialog qbDialog = dataManager.getQBChatDialogDataManager().getByDialogId(dialogId);
      Message message = dataManager.getMessageDataManager().getByMessageId(messageId);
//      DbUtils.updateStatusMessageLocal(dataManager, messageId, State.READ);
      ...
      Log.d("testt",""+message);
      if (qbDialog!=null) {
        if(qbDialog.isPrivate()){
          DbUtils.updateStatusMessageLocal(dataManager, messageId, State.READ);
        }else {
          if (!seenCountMap.containsKey(messageId)) {
            seenCountMap.put(messageId, seenUsersCount + 1);
          } else {
            seenCountMap.put(messageId, seenCountMap.get(messageId) + 1);
          }

          if (seenCountMap.get(messageId) == qbDialog.getOccupants().size()-1) {
            seenCountMap.remove(messageId);
            DbUtils.updateStatusMessageLocal(dataManager, messageId, State.READ);
          }
        }
      }
  ...
  private class GroupChatMessagesStatusListener implements QBMessageStatusListener {



    @Override
    public void processMessageDelivered(String messageId, String s1, Integer integer) {
      DbUtils.updateStatusMessageLocal(dataManager, messageId, State.DELIVERED);
    }

    ///////////////////////////STEEEEEEEE////////////////////////////////////
    @Override
    public void processMessageRead(String messageId, String s1, Integer integer) {

      if (seenCountMap.containsKey(messageId)) {
        seenCountMap.put(messageId, seenCountMap.get(messageId) + 1);
        if (seenCountMap.get(messageId) == currentDialog.getOccupants().size()) {
          DbUtils.updateStatusMessageLocal(dataManager, messageId, State.READ);
        }else {
          DbUtils.updateStatusMessageLocal(dataManager, messageId, State.DELIVERED);
        }
  ...
  private class ParticipantListener implements QBChatDialogParticipantListener {
    ...
    public void processPresence(String dialogId, QBPresence presence) {
      boolean validData = currentDialog != null && presence.getUserId() != null
          && currentDialog.getRoomJid() != null;
      if (validData && currentDialog.getRoomJid()
          .equals(JIDHelper.INSTANCE.getRoomJidByDialogId(dialogId))) {
        notifyUpdatingDialogDetails(presence.getUserId(),
            QBPresence.Type.online.equals(presence.getType()));
      }
    }
  }
}