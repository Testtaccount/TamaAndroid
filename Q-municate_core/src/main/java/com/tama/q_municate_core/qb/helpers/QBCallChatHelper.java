...
//        qbRtcClient.setCameraErrorHendler(new CameraErrorHandler());
    ...
    private void startCallActivity(QBRTCSession qbRtcSession) {
        ...
        if (user != null) {
            ...
            List<QMUser> qmUsersList = new ArrayList<>(1);
            qmUsersList.add(user);

            Intent intent = new Intent(context, activityClass);
            intent.putExtra(QBServiceConsts.EXTRA_OPPONENTS, (Serializable) qbUsersList);
            intent.putExtra(QBServiceConsts.EXTRA_QM_LIST, (Serializable) qmUsersList);
            intent.putExtra(QBServiceConsts.EXTRA_CHAT_TYPE, QBDialogType.PRIVATE);
