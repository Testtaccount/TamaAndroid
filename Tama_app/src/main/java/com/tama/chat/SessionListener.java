...
import com.tama.chat.service.SessionJobService;
  ...
  private static class QBSessionListener extends QBSessionListenerImpl {
    ...
    public void onProviderSessionExpired(String provider) {
      ...
      if (QBProvider.FIREBASE_PHONE.equals(provider)
          || QBProvider.TWITTER_DIGITS
          .equals(provider)) { //for correct migration from TWITTER_DIGITS to FIREBASE_PHONE

        FirebaseAuthHelper
            .getIdTokenForCurrentUser(new FirebaseAuthHelper.RequestFirebaseIdTokenCallback() {
              @Override
              public void onSuccess(String authToken) {
                Log.d("mytest", "onSuccess authToken: " + authToken);
