...
import com.tama.chat.ui.fragments.call.ConversationCallFragment;
import com.tama.chat.ui.fragments.call.IncomingCallFragment;
...
import com.tama.q_municate_core.qb.helpers.QBCallChatHelper;
...
import com.tama.q_municate_core.utils.call.SettingsUtil;
...
public class CallActivity extends BaseLoggableActivity implements QBRTCClientSessionCallbacks,
  ...
  public static void start(Activity activity, List<QBUser> qbUsersList,
      ...
      QBDialogType currentChatDialogType,
      List<QMUser> qmUserList) {
    Intent intent = new Intent(activity, CallActivity.class);
    intent.putExtra(QBServiceConsts.EXTRA_OPPONENTS, (Serializable) qbUsersList);
    intent.putExtra(QBServiceConsts.EXTRA_CHAT_TYPE,currentChatDialogType);
    intent.putExtra(QBServiceConsts.EXTRA_QM_LIST, (Serializable) qmUserList);
  ...
  //(QBRTCSessionDescription) getArguments().getSerializable(QBServiceConsts.EXTRA_SESSION_DESCRIPTION);
  ...
  @Override
  public void onUserNotAnswer(QBRTCSession session, Integer userID) {//1111111111111111111
  ...
  public void onReceiveHangUpFromUser(final QBRTCSession session, final Integer userID,
    ...
    if (session.equals(getCurrentSession())) {//anjateluc stega galis
      ...
      ToastUtils.longToast("User " + participantName + " " + getString(
          R.string.call_hung_up) + " conversation");
            if(session.getOpponents().size()==1){//Avetik
                finish();
            } //Hayk
//            finish();//Avetik
    }
  ...
  public void onSessionStartClose(final QBRTCSession session) {
    ...
    if (currentFragment instanceof ConversationCallFragment && session
...
//    finish();//Hayk Avetik
  ...
  private void initFields() {
    ...
    dialogType = (QBDialogType) getIntent().getExtras().getSerializable(QBServiceConsts.EXTRA_CHAT_TYPE);
    qmUserList = (List<QMUser>) getIntent().getExtras().getSerializable(QBServiceConsts.EXTRA_QM_LIST);
    qbConferenceType = (QBRTCTypes.QBConferenceType) getIntent().getExtras().getSerializable(QBServiceConsts.EXTRA_CONFERENCE_TYPE);
    startConversationReason = (StartConversationReason) getIntent().getExtras()
        .getSerializable(QBServiceConsts.EXTRA_START_CONVERSATION_REASON_TYPE);
    qbRtcSessionDescription = (QBRTCSessionDescription) getIntent().getExtras()
        .getSerializable(QBServiceConsts.EXTRA_SESSION_DESCRIPTION);
  ...
  public QMUser getOpponentAsUserFromDB(int opponentId) {
    ...
    return friend!=null?friend.getUser():null;
  }

  //chka avelacrac
  public List<QBUser> getOpponentAsUserFromDBServer(List<Integer> opponentIds) {//Avetik
    List<QBUser> qbUserList = new ArrayList<>();
    DataManager dataManager = DataManager.getInstance();
    List<Friend> userFriendList = dataManager.getFriendDataManager().getAllByIds(opponentIds);
    for (Friend fr : userFriendList) {
      qbUserList.add(UserFriendUtils.createQbUser(fr.getUser()));
    }
    return qbUserList;
  }
  ...
  private void addIncomingCallFragment(QBRTCSessionDescription qbRtcSessionDescription) {
    ...
    if (isInFront) {
      ...
      bundle.putSerializable(QBServiceConsts.EXTRA_CHAT_TYPE, dialogType);
      bundle.putIntegerArrayList(QBServiceConsts.EXTRA_OPPONENTS, new ArrayList<>(qbRtcSessionDescription.getOpponents()));
      bundle.putSerializable(QBServiceConsts.EXTRA_QM_LIST, new ArrayList<>(qbRtcSessionDescription.getOpponents()));
...
//Hayk
    List<Integer> opponentsId = new ArrayList<>();

    for (QBUser qbUser : opponentsList) {
      opponentsId.add(qbUser.getId());
    }


    List<QBUser> userList = getOpponentAsUserFromDBServer(opponentsId);
    SettingsUtil.setSettingsStrategy(this, userList);
    if (qbCallChatHelper != null) {
      qbCallChatHelper.initCurrentSession(newSessionWithOpponents, this, this);

      ConversationCallFragment fragment = ConversationCallFragment.newInstance(opponentsList, opponentsList.get(0).getFullName(), qbConferenceType,
              StartConversationReason.OUTCOME_CALL_MADE,
              qbCallChatHelper.getCurrentRtcSession().getSessionID(),dialogType,qmUserList);
...
//                                opponentsList);//Avetik

        List<QBUser> userList = getOpponentAsUserFromDBServer(opponentsWithoutMe);//Avetik

        SettingsUtil.setSettingsStrategy(this, userList);
        ConversationCallFragment fragment = ConversationCallFragment.newInstance(userList,
            UserFriendUtils.getUserNameByID(session.getCallerID(), userList),
            session.getConferenceType(), StartConversationReason.INCOME_CALL_FOR_ACCEPTION,
            session.getSessionID(), dialogType, qmUserList);
  ...
  private void showToastDeniedPermissions(String[] permissions, int[] grantResults) {
    ...
    ToastUtils.longToast(
        getString(R.string.denied_permission_message,
            StringUtils.createCompositeString(deniedPermissions)));
  }

  public void addTCClientConnectionCallback(
      QBRTCSessionConnectionCallbacks clientConnectionCallbacks) {
    if (getCurrentSession() != null) {
  ...
  private class AudioStreamReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if (intent.getAction().equals(AudioManager.ACTION_HEADSET_PLUG)) {
        ...
        Log.d(TAG,
            "ACTION_SCO_AUDIO_STATE_UPDATED " + intent.getIntExtra("EXTRA_SCO_AUDIO_STATE", -2));
      }
