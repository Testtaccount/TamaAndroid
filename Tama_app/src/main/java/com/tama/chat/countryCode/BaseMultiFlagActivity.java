...
public abstract class BaseMultiFlagActivity extends TamaAccountBaseActivity {
    ...
    protected String mLastEnteredPhoneFirst;
    protected String mLastEnteredPhoneSecond;
    protected CustomPhoneNumberFormattingTextWatcher textWatcherFirst,textWatcherSecond;
    protected CountryAdapter mAdapterFirst, mAdapterSecond;

    protected boolean editFirstNumber = false;
    protected boolean editSecondNumber = false;
    ...
    protected AdapterView.OnItemSelectedListener getOnItemSelectedListener(final Spinner spinner,final EditText editPhoneText,final boolean isFirstNumber){
        return new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(isFirstNumber) {
                    ...
                    editPhoneText.getText().clear();
                    editPhoneText.getText().insert(editPhoneText.getText().length() > 0 ? 1 : 0, String.valueOf(c.getCountryCode()));
                    editPhoneText.setSelection(editPhoneText.length());
                    ...
                    if (!editSecondNumber && mLastEnteredPhoneSecond != null && mLastEnteredPhoneSecond.startsWith(c.getCountryCodeStr())) {
                        return;
                    }else if(editSecondNumber && getSecondNumber()!=null&&!getSecondNumber().isEmpty()){
    ...
    protected OnPhoneChangedListener getOnPhoneChangedListener(final Spinner spinner,final PhoneNumberUtil mPhoneNumberUtil, final boolean isFirstNumber){
...
//    protected AdapterView.OnItemSelectedListener mOnItemSelectedListenerFirst = new AdapterView.OnItemSelectedListener() {
//        @Override
//        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
...
//    };
    ...
    protected void initUI() {
        countryCodeSpinnerFirst.setOnItemSelectedListener(getOnItemSelectedListener(countryCodeSpinnerFirst,enterPhoneNumberTextFirst,true));
        countryCodeSpinnerSecond.setOnItemSelectedListener(getOnItemSelectedListener(countryCodeSpinnerSecond,enterPhoneNumberTextSecond,false));

        mAdapterFirst = new CountryAdapter(this);
        mAdapterSecond = new CountryAdapter(this);

        countryCodeSpinnerFirst.setAdapter(mAdapterFirst);
        countryCodeSpinnerSecond.setAdapter(mAdapterSecond);
        textWatcherFirst = new CustomPhoneNumberFormattingTextWatcher(getOnPhoneChangedListener(countryCodeSpinnerFirst,mPhoneNumberUtilFirst,true));
        textWatcherSecond = new CustomPhoneNumberFormattingTextWatcher(getOnPhoneChangedListener(countryCodeSpinnerSecond,mPhoneNumberUtilSecond,false));
        enterPhoneNumberTextFirst.addTextChangedListener(textWatcherFirst);
        enterPhoneNumberTextSecond.addTextChangedListener(textWatcherSecond);
        ...
        enterPhoneNumberTextSecond.setFilters(new InputFilter[]{filter});
        enterPhoneNumberTextSecond.setImeOptions(EditorInfo.IME_ACTION_SEND);
        enterPhoneNumberTextSecond.setImeActionLabel(getString(R.string.label_send), EditorInfo.IME_ACTION_SEND);
    }

    protected void initCodes() {
        new AsyncPhoneInitTask(this,countryCodeSpinnerFirst, mPhoneNumberUtilFirst,mAdapterFirst).execute();
        new AsyncPhoneInitTask(this,countryCodeSpinnerSecond, mPhoneNumberUtilSecond,mAdapterSecond).execute();
    }
    ...
    protected class AsyncPhoneInitTask extends AsyncTask<Void, Void, ArrayList<Country>> {
        ...
        public AsyncPhoneInitTask(Context context,Spinner spinner,PhoneNumberUtil mPhoneNumberUtil,CountryAdapter mAdapter) {
            mContext = context;
...
//    }
    ...
    {
        BALANCE,PHONE_NUMBER_FIRST,PHONE_NUMBER_SECOND,TRANSFER,SEND_REQUEST
    }
